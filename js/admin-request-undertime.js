...
(function(){
  ...
  async function submitUndertime(){
    ...
    try{
      ...
      alert('Failed to submit undertime');
    }
  }
  ...
  function render(){
    ...
    if (!pageRows.length){
      ...
      pageRows.forEach((it, idx) => {
        ...
        if (approveBtn) approveBtn.addEventListener('click', async () => { await submitDecision(it, 'approve'); await loadUndertime(); render(); });
        if (rejectBtn) rejectBtn.addEventListener('click', async () => { await submitDecision(it, 'reject'); await loadUndertime(); render(); });
        tbody.appendChild(tr);
      });
    }
